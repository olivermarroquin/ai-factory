# Jobs API Framework Adapter Scope v1

## Purpose

Define the minimal contract for wiring `backend/api/jobs.py:handle()` to a real HTTP route. This document is the authoritative scope for the framework adapter implementation. No code is written until this scope is reviewed and accepted.

---

## Scope

This scope covers exactly one thing: exposing `POST /jobs/parse` through a Flask route that delegates entirely to `handle()`.
...
- Follows the same pattern as the accepted `rewrite_routes.py` and `resume_routes.py` adapters.
...
- Route is defined in `backend/api/jobs_routes.py` (new file, adapter only).
- Route registers a Blueprint named `jobs_bp` with no URL prefix.
- The single route is `POST /jobs/parse`.
- Route function calls `handle()` and returns its output as a Flask JSON response.
...
| Missing/empty `job_description` | `handle()` — `400 missing_field` |
| Service exception | `handle()` — `500 service_error` |
...
- Blueprint URL prefix (none — route is exactly `/jobs/parse`)
...
1. `docs/jobs-api-framework-adapter-scope-v1.md` is written and reviewed.
2. The scope is accepted as the contract for `backend/api/jobs_routes.py`.
3. Implementation of `backend/api/jobs_routes.py` begins only after acceptance.
