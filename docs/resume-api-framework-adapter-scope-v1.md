# Resume API Framework Adapter Scope v1

## Purpose

Define the minimal contract for wiring `backend/api/resume.py:handle()` to a real HTTP route. This document is the authoritative scope for the framework adapter implementation. No code is written until this scope is reviewed and accepted.

---

## Scope

This scope covers exactly one thing: exposing `POST /resume/parse` through a Flask route that delegates entirely to `handle()`.
...
- Follows the same pattern as the accepted `rewrite_routes.py` adapter.
...
- Route is defined in `backend/api/resume_routes.py` (new file, adapter only).
- Route registers a Blueprint named `resume_bp` with no URL prefix.
- The single route is `POST /resume/parse`.
...
| Missing/empty `resume_text` | `handle()` — `400 missing_field` |
...
- Blueprint URL prefix (none — route is exactly `/resume/parse`)
...
1. `docs/resume-api-framework-adapter-scope-v1.md` is written and reviewed.
2. The scope is accepted as the contract for `backend/api/resume_routes.py`.
3. Implementation of `backend/api/resume_routes.py` begins only after acceptance.
