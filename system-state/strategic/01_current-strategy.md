...
- Synchronized three-pane review experience — scroll-sync across Original, Proposed, and Proposals panes so all three move together; cursor-position highlighting that shows where an edit is happening across all three panes simultaneously (cross-pane anchor highlights). Surfaced as a product idea during Stage 4a live test 2026-04-22. Related to but more ambitious than the pane-scroll-alignment item above — that one was line-by-line static alignment; this is dynamic synchronization with anchor highlighting. Deliberately deferred from MVP because: (a) not in the frontend-mvp-spec-v1.md spec; (b) scroll-sync alone is straightforward, but cross-pane anchor highlighting is a meaningful piece of work requiring cross-pane DOM coordination and a shared logical "position" concept across text-based and card-based panes; (c) real users haven't encountered the MVP yet to confirm this is a real usability win versus a nice-to-have. Revisit trigger: after MVP ships and real usage (operator's own job applications, or client demos) reveals whether pane-coordination friction is a top-5 user complaint. If yes, design pass before implementation. If no, stays deferred indefinitely.
